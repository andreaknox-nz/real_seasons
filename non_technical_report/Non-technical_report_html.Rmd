...
You can then average the coefficients to get a measure of the extent to which days are appropriately classified into their closest seasons. Higher average values indicate more appropriate groupings and, therefore, a better seasonal classification.
...
In k-means clustering we first specify the number of clusters we want. We then use the k-means clustering algorithm to generate that number of clusters. It works as follows. 
...
We used this technique to generate 4 clusters (for comparison with conventional seasons) and 5 clusters (for comparison with 'real seasons). Since the clusters were generated from weather data directly, they should be fairly optimal groupings of days with similar weather patterns. Almost certainly they will be more optimal than the seasons. Were they? We can check by looking at the silhouette plots.
...
Note that the size of the groups is uneven. We see that there is one group with far fewer days than the others, both for the 4 clusters (group 2) and the 5 clusters (group 3).


### Conventional seasons are more similar than 'real' seasons to k-means clusters

Now that we have our more optimal groupings, we can compare the seasonal classifications to them and see which is closer to optimal. In this analysis we think of the k-means clusters as "correct" and we test how good the conventional and 'real' seasons are at placing days into their "correct" groups.

First we had to figure out how to match seasons to groups. We want both classifications to have the best possible chance of generating correct predictions, so we tried all possible matches between seasons and groups and chose the concordance that returned the highest number of "correctly" classified days. The best concordances were:

* Conventional seasons: Spring = group 1, Summer = group 4, Autumn = group 2, Winter = group 3.
* Real seasons: Spring1 and 2 = group 3, Shitsville = group 1, Summer = group 5, Autumn = group 4, Winter = group 2.

We then used four different performance metrics to compare the ability of the conventional and 'real' seasons to make "correct" prediction, where, for example in the conventional seasons, a spring day is correctly classified if it is in group 1, Summer days are correct if they are in group 4, Autumn in group 2 and Winter in group 3. The first of our performance measures, overall accuracy, is straightforward. It is simply the overall percentage of correct predictions made by the seasonal classification. The higher the percentage the better. The other three are a little more involved, so we do not explain them here, but you can find a full description in our [technical report](https://github.com/andreaknox-nz/real_seasons/blob/main/final_report%20(technical)/Final_report.pdf). All you really need to know is that for all 4 measures, bigger is better. The classification with higher percentage values is closer to the "optimal" k-means clusters.
...
What we see is that, on every performance measure, the conventional seasons do better than the 'real' seasons. 

However, we need to be a bit careful because performance here depends not just on how good the seasons are, but also on the how good the k-means clustering is. We might get a different result if we used a different clustering method. Nevertheless, the result is consistent with our other lines of investigation and so this is one more piece of evidence that the conventional seasons may be better than 'real' seasons.

Similar to what we saw above, however, these is another aspect to this result. In fact neither seasonal classification is that close top optimal, with even the better one, the conventional seasons, making less than 50% "correct" predictions.


### Unfortunately our evidence doesn't support an official change to the seasons


Well, shit.

Unfortunately we cannot make the case for an official change to how we define New Zealand or even just Wellington seasons.

If this is not the result you are looking for, we're sorry. It's not the result we were looking for either, but as statisticians we know that statistics ~~don't lie~~ are open to interpretation but generally more reliable than anecdata.

Something about seasons being to do with the tilt of the sun.

### Where to from here?
Renaming of spring to Shitsville is not the problem. It's the creation of an extra season (Spring 1 and 2) that is not supported by the data.
Shitsville is still potentially valid, because we all know how bad the weather is at that time of year. 
But one thing we saw from the distance analysis is that Autumn is actually very similar to Spring/Shitsville.

So here's an alternative proposal:

Shitsville 1, Shitsville 2, Really Shit, Quite Nice Sometimes.


Our future work might investigate whether there's any supporting evidence for this. If we get time. We probably won't because, unfortunately, this kind of research doesn't pay the bills.
